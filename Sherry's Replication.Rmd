...
figure4_descriptive <- figure4 %>%
  mutate(
    Region = case_when(
      grepl("Nose", Type) ~ "Nose",
      grepl("Ear", Type) ~ "Ear"
    ),
    Condition = case_when(
      grepl("Aversive", Type) ~ "Aversive",
      grepl("NonAversive", Type) ~ "NonAversive"
    ),
    Period = ifelse(Time < 0, "Before", "After")
  )

# --- Calculate means and SDs for each Region x Condition x Period group
descriptive_stats_fig4 <- figure4_descriptive %>%
  group_by(Region, Condition, Period) %>%
  summarise(
    n = n(),
    mean_temp = round(mean(Temperature, na.rm = TRUE), 2),
    sd_temp = round(sd(Temperature, na.rm = TRUE), 2)
  ) %>%
  arrange(Region, Condition, Period)

descriptive_stats_fig4
